...
: ior  ( n -- ior ) dup 0 >  if  drop  0  else
  ...
  then ;
: open-file  ( a n fam -- fd ior ) >r zstring r> OPEN_MODE 
  sys_open syscall3 dup ior ;
: create-file  ( a n fam -- fd ior ) >r zstring r> O_TRUNC or 
  OPEN_MODE sys_open  syscall3 dup ior ;
...
: lookup  ( a u dp -- dp2 | 0 ) begin  ?dup  while
    >r  2dup r@ count significant compare 0=  if  2drop  r>  |
